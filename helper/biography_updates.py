...
distinct from historical facts. Only events and new historical/life episodes in
...
def plan(memory, previous, payload, records, request):
    ...
    significant = assess(old, payload, additions, new_episodes)
    if not significant:
    ...
    if not additions and not new_episodes:
